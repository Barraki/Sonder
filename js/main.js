...
$(document).ready(function () {
	$("#nav-bar_footer").on("click", "a", function (event) {
...
$(document).ready(function () {
	$("#nav-bar-mobile").on("click", "a", function (event) {
		event.preventDefault();
		var id = $(this).attr('href'),
			top = $(id).offset().top - 100;
		$('body,html').animate({
...
$("#form").submit(function () { //Change
	...
	$.ajax({
		...
		alert("Спасибо что оставили заяку. Наш менеджер свяжется с вами в ближайшее время!");
		setTimeout(function () {
			// Done Functions
			th.trigger("reset");
		}, 1000);
...
$("#form1").submit(function () { //Change
	...
	$.ajax({
		...
		alert("Cпасибо что оставили заявку, наш менеджер свяжется с вами в ближайшее время!");
		setTimeout(function () {
			// Done Functions
			th.trigger("reset");
		}, 1000);
...
$(window).scroll(function () {
	if ($(this).scrollTop() > 1) {
		$('header').addClass("main-header-box");
	} else {
		$('header').removeClass("main-header-box");
	}
});
...
$(document).ready(function () {
	...
	$("#phone-page").mask("+7(999) 999-99-99");
});
